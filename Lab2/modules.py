...
Original file is located at
    https://colab.research.google.com/drive/1xGzXyjj6z1IEYAJAaRnv5HkbTkzPssYJ
...
class Sequential(Module):
    ...
    def backward(self, input, gradOutput):
        ...
        # Your code goes here. ################################################
        self.gradInput = gradOutput
        for m in range(len(self.modules))[::-1]:
            if m == 0:
                self.gradInput = self.modules[m].backward(input, self.gradInput)
            else:
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = gradOutput - np.multiply(input/np.sum(input, axis=1, keepdims=True),
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if (self.training == True):
            ...
            self.output = np.divide((input - self.mean), np.sqrt(self.var + self.EPS))
        else:
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = input.shape[0] * gradOutput - np.sum(gradOutput, axis=0, keepdims=True) - self.output * np.sum(gradOutput * self.output, axis=0, keepdims=True)
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        ...
        if not self.training:
            ...
            self.mask = np.divide(np.random.binomial(1, 1 - self.p, input.shape).astype(float), (1 - self.p))
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.add(np.maximum(input, 0), self.slope * np.minimum(input, 0))
        return  self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.multiply(((input >= 0) + self.slope * (input < 0)), gradOutput)
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.add(np.maximum(input, 0), self.alpha * np.expm1(np.minimum(input, 0)))
        return  self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = ((input > 0) + (input <= 0) * self.alpha + self.alpha * np.expm1(np.minimum(input, 0))) * gradOutput
        return self.gradInput
...
class SoftPlus(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.divide(np.multiply(gradOutput, np.exp(input)), (1 + np.exp(input)))
        return self.gradInput
...
class MSECriterion(Criterion):
    ...
    def updateOutput(self, input, target):   
        self.output = np.divide(np.add(np.power(input - target, 2)), input.shape[0])
        return self.output 
 
    def updateGradInput(self, input, target):
        self.gradInput  = np.divide((input - target) * 2, input.shape[0])
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target): 
        ...
        # Your code goes here. ################################################
        self.output = np.divide(-np.sum(target * np.log(input_clamp)), input.shape[0])
        return self.output
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target): 
        # Your code goes here. ################################################
        self.output = np.divide(-np.sum(target * input), input.shape[0])
        return self.output
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        # self.output = ...
        padded_inp = np.pad(input, ([0, 0], [0, 0], [pad_size, pad_size], [pad_size, pad_size]))

        self.output = np.zeros([input.shape[0], self.out_channels, input.shape[2], input.shape[3]])
        for inp in range(input.shape[0]):
            for out in range(self.out_channels):
                self.output[inp][out] = scipy.signal.correlate(padded_inp[inp], self.W[out], mode='valid')[0] + self.b[out]
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        # self.gradInput = ...
        padded_otp = np.pad(gradOutput, ([0, 0], [0, 0], [pad_size, pad_size], [pad_size, pad_size]))

        self.gradInput = np.zeros(input.shape)
        for inp in range(input.shape[0]):
            for chan in range(self.in_channels):
                self.gradInput[inp][chan] = scipy.signal.correlate(padded_otp[inp], self.W[:, :, ::-1, ::-1][:, chan], mode='valid')[0]
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        for inp in range(self.in_channels):
            for out in range(self.out_channels):
                self.gradW[out][inp] = scipy.signal.correlate(padded_inp[:, inp], gradOutput[:, out], mode='valid')[0]
...
class MaxPool2d(Module):
    ...
    def updateOutput(self, input):
        ...
        # self.output = ...
        # self.max_indices = ...
        reshaped_input = np.swapaxes(input.reshape(input.shape[0], input.shape[1], input_h // self.kernel_size,
                                          self.kernel_size, input_w // self.kernel_size, self.kernel_size), 3, 4)
        self.output = np.max(reshaped_input, axis=(4, 5))
        self.max_indices = (reshaped_input - self.output[:, :, :, :, np.newaxis, np.newaxis]) == 0
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        # YOUR CODE #############################
        # self.gradInput = ...
        self.gradInput = np.swapaxes(gradOutput[:, :, :, :, np.newaxis, np.newaxis] * self.max_indices, 3, 4).reshape(input.shape)
        return self.gradInput
